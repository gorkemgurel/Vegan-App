                            ...
                            .background(GeometryReader { scrollGeometry in
                                Color("RecipeViewSelectedTabColor")
                                    .onChange(of: scrollGeometry.frame(in: .global).minY) { minY in
                                        ...
                                        //testOffset = max(0, -geometry.frame(in: .global).minY)
                                        //blurAmount = 0
                                        //opacityAmount = 0
                                    }
                            })
...
extension View {
    func stickyTab() -> some View {
        modifier(StickyTab())
    }
}

struct StickyTab: ViewModifier {
    @State private var frame: CGRect = .zero
    @Environment(\.safeAreaInsets) private var safeAreaInsets
    
    var isSticking: Bool {
        frame.minY < safeAreaInsets.top + 44
    }
    
    func body(content: Content) -> some View {
        content
            .background(isSticking ? Color("RecipeViewStickyColor") : Color("RecipeViewSelectedTabColor"))
            .offset(y: isSticking ? -frame.minY + safeAreaInsets.top + 44 : 0)
            .zIndex(isSticking ? .infinity : 0)
            .overlay(GeometryReader { proxy in
                let f = proxy.frame(in: .named("container"))
                Color.clear
